...
| **File Viewer** | 原生查看 PNG/JPG/SVG/EPS 图片（含缩放）；PDF/Office 调用系统默认程序 |
...
应用程序支持在标签页中直接查看多种文件格式。

### 支持格式

| 类别 | 扩展名 | 查看方式 |
|------|--------|---------|
| **光栅图片** | PNG, JPG, JPEG, BMP, GIF, WebP, ICO | 可滚动原生查看 + 缩放控件（− / + / Fit / 1:1） |
| **矢量 — SVG** | SVG | QSvgWidget 矢量渲染 |
| **矢量 — EPS** | EPS, EPSF, PS | Ghostscript → PNG（自动检测 PATH、TeX Live 或捆绑的 GS）；QPixmap 回退 |
| **PDF 文档** | PDF | 调用系统默认 PDF 阅读器 |
| **Office 文档** | DOC, DOCX, XLS, XLSX, PPT, PPTX | 调用系统默认程序 |
...
1. **QPixmap** — Qt 可直接加载部分 EPS
2. **Ghostscript** — 自动搜索 PATH、标准安装路径、TeX Live 目录及捆绑的 GS；转 EPS 为临时 PNG
3. **错误提示** — 无渲染器时状态栏提示安装 Ghostscript

### 其他程序打开

在**算例浏览器**中右键任意文件，选择 **Open With...** 可选择外部程序打开该文件。
...
- **行号** — 当前行高亮
- **多标签页** — `Ctrl+W` 关闭当前标签页；`Ctrl+Tab` 切换标签页
- **标签页右键菜单** — 右键任意标签页可关闭当前 / 关闭其他 / 关闭全部
- **未保存更改** — 标签页标题 `*` 标记 + 关闭确认
...
- **过滤器**快速搜索文件
- **右键上下文菜单**：
  - 打开文件 / **Open With...**（选择外部程序） / 关闭算例 / 刷新算例
  - 新建文件 / 新建文件夹 / 删除
- **多算例支持** — 可同时打开管理多个算例
...
- **Qt 模块**：Core, GUI, Widgets, SvgWidgets（SVG 矢量图渲染）
...
2. **File → Open Case** 已改为 **Case → Open Case**（`Ctrl+O`），选择一个 OpenFOAM 算例目录
...
│   ├── fileviewer.h / .cpp            # 文件查看器：图片渲染、GS 检测、缩放控件
